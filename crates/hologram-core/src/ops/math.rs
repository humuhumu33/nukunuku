...
fn try_inline_vector_add<T: bytemuck::Pod>(
    ...
    // Note: Pointers are valid for n elements as guaranteed by buffer allocation
...
pub fn min<T: bytemuck::Pod>(
    ...
    // Build ISA program: c[i] = min(a[i], b[i])
    let ty = crate::isa_builder::type_from_rust_type::<T>();
    let program =
        crate::isa_builder::build_elementwise_binary_op(handle_a, handle_b, handle_c, n, ty, |dst, src1, src2| {
...
pub fn max<T: bytemuck::Pod>(
    ...
    // Build ISA program: c[i] = max(a[i], b[i])
    let ty = crate::isa_builder::type_from_rust_type::<T>();
    let program =
        crate::isa_builder::build_elementwise_binary_op(handle_a, handle_b, handle_c, n, ty, |dst, src1, src2| {
...
pub fn relu<T: bytemuck::Pod>(exec: &mut Executor, a: &Buffer<T>, c: &mut Buffer<T>, n: usize) -> Result<()> {
    ...
    for i in 0..n {
        ...
        // Create zero in r2 using MOV_IMM (avoids XOR which doesn't support f32)
...
///
/// Uses operation-level chunking for large vectors.
pub fn relu_par<T: bytemuck::Pod + Send + Sync>(
    exec: &mut Executor,
    input: &Buffer<T>,
    output: &mut Buffer<T>,
    n: usize,
